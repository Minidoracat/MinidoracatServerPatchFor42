# -*- coding: utf-8 -*-
"""由 STEAM_DESCRIPTION.md 重新生成 workshop.txt 的 description 區塊。
...
- 描述每行加 description= 前綴（空行也輸出 description=），工作區行尾 CRLF、UTF-8 無 BOM
- 本 MOD 是純中文單檔受眾：唯一描述來源就是 STEAM_DESCRIPTION.md（沒有 _EN/_JP）

首發流程：先跑本腳本預建（無 id=）→ 遊戲上傳並回寫 id= → 再跑本腳本補描述 → commit。
之後每次更新：改 STEAM_DESCRIPTION.md → 跑本腳本（id 會被保留）→ 上傳。
"""
import os
import re

REPO = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
SRC = os.path.join(REPO, "STEAM_DESCRIPTION.md")   # 純中文單檔受眾
DST = os.path.join(REPO, "MOD", "MinidoracatServerPatchFor42", "workshop.txt")

TITLE = "Minidoracat Server Patch for B42"
TAGS = "Build 42;Multiplayer"
VISIBILITY = "unlisted"   # 本服專用補丁，不對外推廣
